...
class ResNetG(nn.Module):
    ...
    def __init__(
        ...
        self.layer1m = self._make_layerm(block, 256, layers[0], stride=2)

        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer2m = self._make_layerm(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
    ...
    def _make_layerm(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False) -> nn.Sequential:
        norm_layer = self._norm_layer
        upsample = None
        previous_dilation = self.dilation
    ...
    def attention(self, query, key, value):
        ...
        dim_key = key.shape[-1]
        scaled_scores = scores / math.sqrt(dim_key)
        attention_weights = torch.softmax(scaled_scores, dim=-1)
        output = torch.matmul(attention_weights, value)
        return output
    def _forward_impl(self, x: Tensor,m:Tensor) -> Tensor:
        ...
        feature_a_m = self.layer1m(m)

        attention_a_m  = self.attention(feature_a, feature_a_m, feature_a)

        feature_b = self.layer2(attention_a_m)  # 256*16*16->128*32*32

        feature_b_m = self.layer2m(feature_a_m)

        attention_b_m = self.attention(feature_b, feature_b_m, feature_b)

        feature_c = self.layer3(attention_b_m)  # 128*32*32->64*64*64
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        #self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        # self.layer1 = self._make_layer(block, 256, layers[0], stride=2)
        self.layer2 = self._make_layerD(block, 64, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layerU(block, 64, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        #self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
        #                               dilate=replace_stride_with_dilation[2])
        #self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def _make_layerD(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False) -> nn.Sequential:
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
    ...
    def _make_layerU(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False) -> nn.Sequential:
        norm_layer = self._norm_layer
        upsample = None
        previous_dilation = self.dilation
...
def _resnetGT(
    ...
    **kwargs: Any
    ...
    model = ResNetG(block, layers, **kwargs)
...
def resnext101_32x8d(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnext101_32x8d', Bottleneck, [3, 4, 23, 3],
                   pretrained, progress, **kwargs)
def de_wide_resnet50_2G(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnetGT('wide_resnet50_2', Bottleneck, [3, 4, 6, 3],
                   pretrained, progress, **kwargs)
